...
仓内路径：`app/src/main/assets/pet/live2d/Mao/`（约 4.2MB，**可 commit**）。
...
2. `BuiltInLive2dAssets.ensureInstalled(context)` → `filesDir/builtin-live2d/Mao/`
...
4. `Live2dDisplayController`：逻辑路径 → `LIVE2D_SHELL`（`file:///android_asset/...`）；落盘后 → `live2d_shell_ready`
5. 设置页：`已就绪（内置示例）`
