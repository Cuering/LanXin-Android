...
> 当前状态：**Phase 1 ✅ → Phase 2 ✅ → Phase 3 ✅ → Phase 4 ✅ → Phase 5.1–5.7 ✅ → Phase 6.1 🚧**
> - Step①~⑧ 全部完成
...
> - Phase 6.1：MNN 本地推理引擎骨架（接口 + stub + 配置 + 路由选择）🚧（`feat/phase6-1-mnn-local-inference`）
...
│   └── local_inference/    本地推理（MNN 骨架）🚧 6.1
│
...
| `builtin/local_inference/` | 🚧 Phase 6.1 | 高 | MNN 骨架：接口 + stub + 配置（见第十四节） |
...
| `docs/local-inference.md` | 本地推理引擎骨架与 MNN 接入点 | 🚧 6.1 |
| `builtin/local_inference/README.md` | 本地推理模块说明 | 🚧 6.1 |
...
| 非目标 | 打包模型文件、真实 MNN so、6.2 自动离线切换、6.3 ChatRouter 重构 |
