...
> - Step⑦ 知识库（Knowledge） + 记忆向量检索（Phase 3 进行中）
...
├── app/                    [壳应用]
│   ├── LanXinApp.kt        初始化入口
│   ├── builtin/            内置功能实现（persona / statistics / scheduler / platform ✅）
...
| `builtin/knowledge/` | 🚀 Phase 3 | 高 | GTE-small(ONNX) + ObjectBox VectorDB 双路 RRF 融合检索 |
...
| 知识库 | 本地 embedding + 向量检索 | 端侧 GTE-small(ONNX) + ObjectBox VectorDB 双路 RRF 融合 |
| 子代理 | 内置核心功能 | 手机端不建议实现 |
...
| `builtin/knowledge/README.md` | 知识库方案（见第十一节） | 🚀 Phase 3 P0 |
...
| 阶段 | 内容 | 目标 |
|------|------|------|
| **P0** | 打通向量管道（ONNX + GTE-small int8 + ObjectBox embed/检索） | 全链路 < 50ms 🚀 |
| **P1** | 改造 MemoryInjector 为双路 RRF 融合，保留关键词回退开关 | 语义检索可用 |
| **P2** | builtin/knowledge 文档导入（txt/md/pdf），512/50 滑动窗口分段 | 知识库功能上线 |
| **P3** | 自动从对话抽取知识点，轻量摘要后向量化入库 | 知识自动积累 |
